...
class CanvasASInterop {
    ...
    render(data) {
        var index = 0;
        var stride = 0;
        while (index < data.length && data[index] !== 6 /* Commit */) {
            stride = data[index + 1];
            switch (data[index]) {
                case 0 /* Arc */: {
    ...
    create_pattern(index, imageIndex, patternType) {
        this.patterns[index] = this.ctx.createPattern(this.images[imageIndex], exports.canvasPatternTypes[patternType]);
    }
